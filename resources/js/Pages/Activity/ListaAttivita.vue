<template>
    <Head title="Lista Attività" />

    <h2 class="text-h3 text-center text-white my-2">Lista Attività</h2>
    <v-data-table
        class="rounded elevation-1 custom-table"
        :items="props.listaAttivita"
        :headers="headers"
    >
            ...
            <div :class="$vuetify.display.smAndDown ? 'flex justify-between' : ''">
                ...
                >
                    <v-card>
                        ...
                        <v-card-text>Sei sicuro di eliminare l'attività con ID: <strong>{{ itemToDelete }}</strong>  ?</v-card-text>
                ...
                >
                    <v-card>
                        <v-card-title>Modifica Attività</v-card-title>
...
const props = defineProps({ listaAttivita: Object });
...
function openDialogEdit(item) {
    ...
    formEdit.tipo = item.tipo;
    formEdit.cost = item.cost;
...
function confirmDelete() {
    form.delete('/eliminaAttivita/'+itemToDelete.value, {
        onSuccess: () => {
...
function confirmEdit() {
    formEdit.patch('/attivita/'+itemToEdit.value, {
        onSuccess: () => {
